...
cmake . -DCMAKE_TOOLCHAIN_FILE=/toolchain.cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=${SYSROOT}/usr -DENABLE_ZIP=OFF -DENABLE_CRYPTO=OFF -DENABLE_DATA=OFF -DENABLE_MONGODB=OFF -DENABLE_NETSSL=OFF -DENABLE_PAGECOMPILER_FILE2PAGE=OFF -DENABLE_PAGECOMPILER=OFF
make -j$(nproc)
make install
